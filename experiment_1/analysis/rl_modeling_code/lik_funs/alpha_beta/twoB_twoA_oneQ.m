...
function [loglik] = twoB_twoA_oneQ(parameters, subj)
...
nd_alpha_c = parameters(3); % normally distributed alpha
alpha_c = 1/(1+exp(-nd_alpha_c)); % alpha (transformed to be between zero and one)

nd_alpha_e = parameters(4); % normally distributed alpha
alpha_e = 1/(1+exp(-nd_alpha_e)); % alpha (transformed to be between zero and one)

nd_Qinit = parameters(5); %normally distributed Qinit
q_init = nd_Qinit/10; 

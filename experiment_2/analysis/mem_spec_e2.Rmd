...
```{r plot learning across age groups lines, fig.height=5}
...
```{r plot learning by block number lines, fig.height=5}

# Plot correct response as a function of reward condition and stimulus repetition 
correct_response_summary_sub <- learning_data %>%
    filter(rt > 0) %>%
    group_by(reward_condition, stimulus_repetition, subject_id, within_condition_block_num) %>%
    summarize(mean_correct = mean(correct_response_made, na.rm = T))

correct_response_summary <- correct_response_summary_sub  %>%
    group_by(reward_condition, stimulus_repetition, within_condition_block_num) %>%
    summarize(group_mean_correct = mean(mean_correct, na.rm = T),
              se_correct = se(mean_correct))
...
```{r plot generalization by age group lines, fig.height=5}
...
correct_response_category_rep_plot <- ggplot() +
    ...
    xlab("Category Repetition") +
    ylab("Correct Responses to 1st Stimulus") +
        scale_color_manual(values = c(color1, color2),
                       name = "Block Condition") + 
    geom_hline(yintercept = .33, linetype = "dashed") +
    mem_theme  +
...
```{r plot generalization by block number lines,  fig.height=5}
...
tab_model_glmer(auc_model_delay.lmer)
```
...
```{r simple choice weights fig, fig.width = 6}
...
```{r choice weights lines fig, fig.height = 5, fig.width = 5}
...
tab_model_glmer(beta.tradeoff.model)
...
tab_model_glmer(beta.tradoff.model.cat_pred)
...
beta_points_model
tab_model_glmer(beta_points_model)
...
auc_points_plot <- ggplot(auc_beta_points) +
    facet_grid(cols = vars(performance_group), rows = vars(abstraction)) +
      geom_hline(yintercept = .5, linetype = "dashed") +
    geom_point(data = auc_beta_points, aes(x = reward_condition, color = abstraction, y = AUC), shape = 95, size = 15, alpha = .2) + 
...
recov_ests <- read_csv("analysis/rl_modeling_code/recov/fourB_oneA_oneQ_CF_param_recov.csv") %>%
    rownames_to_column("sim_num") %>%
    pivot_longer(sim_beta_c_c:sim_q_init,
                 values_to = "sim_val",
                 names_to = "sim_param",
                 names_prefix = "sim_") %>%
    pivot_longer(fit_beta_c_c:fit_q_init,
                 values_to = "fit_val",
                 names_to = "fit_param",
                 names_prefix = "fit_") %>%
...
recov_ests$param <- factor(recov_ests$param, 
                          labels=c(
    'alpha_choice'=parse(text=TeX(' $\\alpha_{choice}$')),
    'alpha_cf'=parse(text=TeX(' $\\alpha_{cf}$')),

...
function [sim_choices, sim_points] = sim_fourB_twoQ(parameters, subj)
...
nd_q_cat = parameters(6); % normally distributed alpha
q_init_cat = 1/(1+exp(-nd_q_cat));

nd_q_ex = parameters(7); % normally distributed alpha
q_init_ex = 1/(1+exp(-nd_q_ex));
...
for trial_num = 1:T
    ...
    if trial_num == 1 || trial(trial_num) < trial(trial_num - 1)
        exemp_ests = q_init_ex .* ones(9, 3);
        cat_ests = q_init_cat .* ones(3, 3);
    end

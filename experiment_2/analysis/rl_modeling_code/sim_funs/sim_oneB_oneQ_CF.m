...
function [sim_choices, sim_points] = sim_oneB_oneQ_CF(parameters, subj)
...
nd_alpha_CF = parameters(3); % normally distributed alpha
alpha_CF = 1/(1+exp(-nd_alpha_CF)); % alpha (transformed to be between zero and one)

nd_q = parameters(4); % normally distributed alpha
q_init = 1/(1+exp(-nd_q));
...
for trial_num = 1:T
    ...
    weighted_exemplar_choice_vals = beta .* exemp_ests(exemplar(trial_num), :);
    weighted_category_choice_vals = beta .* cat_ests(category(trial_num), :);

...
function [sim_choices, sim_points] = sim_twoB_oneQ_CF(parameters, subj)
...
nd_alpha_CF = parameters(4); % normally distributed alpha
alpha_CF = 1/(1+exp(-nd_alpha_CF)); % alpha (transformed to be between zero and one)

nd_q = parameters(5); % normally distributed alpha
q_init = 1/(1+exp(-nd_q));
